...
def train(train_loader, model, criterion, optimizer, debug=False, flip=True):
    ...
    for i, (input, target, meta, img_path) in enumerate(train_loader):
        ...
        if debug: # visualize groundtruth and predictions
            ...
            plt.pause(.05)
            plt.draw()
            plt.savefig('/home/shantam/Documents/Programs/pred.png')
            cv2.imwrite('/home/shantam/Documents/Programs/pred_cv.png', pred_batch_img)
#            time.sleep(5)
...
def validate(val_loader, model, criterion, num_classes, debug=False, flip=True):
    ...
    with torch.no_grad():
        for i, (input, target, meta, img_path) in enumerate(val_loader):
            ...
            preds, vals = final_preds(score_map, meta['center'], meta['scale'], [64, 64])
            ...
            if debug:
                ...
                plt.pause(.05)
                plt.draw()
                cv2.imwrite('/home/shantam/Documents/Programs/pytorch-pose/example/predictions/pred'+str(i)+'.png', image)
                #time.sleep(5)
